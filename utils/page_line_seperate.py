...
class Page():
    ...
    def skew_correct(self, ):
        ...
        # for a in (0,):
            data = inter.rotate(self.imgarr, a, reshape=False, order=0)
...
def main(image_name):
    ...
    page.process()
    print(page.path)
    print("Best Harmonic:", page.best_harmonic)
    print("Line\tlineht\txht\ttextht\ttextwd\txht/lineht\ttextht/lineht")
    for line in range(page.num_lines):
        bottom = page.line_sep[line + 1]
        top = page.line_sep[line]
        ht, wd = ht_wd(page.imgarr[top:bottom, :])
        xht = page.base_lines[line] - page.top_lines[line]
        lineht = bottom - top
        print(f"{line}\t{lineht}\t{xht}\t{ht}\t{wd}\t{xht/lineht:.2f}\t{ht/lineht:.2f}")
